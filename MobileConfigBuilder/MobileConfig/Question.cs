...
namespace MobileConfig
{
    public class Question: ConfigBase, ConfigInterface
    {
        ...
        public Value DefaultValue
        {
            get
            {
                if (_defaultvalue == null && _masterDS != null)
                {
                    DataTable dt = _masterDS.Tables[MobileConfig.Question.TableName];
                    if (dt != null && dt.Columns.Contains("DefaultValueID"))
                    {
                        DataRow[] rows = dt.Select("QuestionID = '" + ID + "'");
                        if (rows.Length > 0)
                        {
                            _defaultvalue = new Value(_masterDS, rows[0]["DefaultValueID"].ToString());                            
                        }
                    }
                }
		...
		public RegValue[] RegValues
		{
			get
			{
				if (_regvalues == null && _masterDS != null)
				{
		...
		public RegValue SelectRegValue
		{
			get
			{
				...
				{
					DataTable dt = _masterDS.Tables[MobileConfig.RegValue.TableName];
					if (dt != null && dt.Columns.Contains("RegValueID"))
					{
						DataRow[] rows = dt.Select("RegValueID = '" + _selregvalueid + "'");
						if (rows.Length > 0)
						{
							_selregvalue = new RegValue(_masterDS, rows[0]["RegValueID"].ToString());
						}
					}
				}
				return _selregvalue;
